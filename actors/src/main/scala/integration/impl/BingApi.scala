...
case class BingApi(server: SearchServer)(implicit ec: ExecutionContext) extends SearchingApi[Future] {

  override val source: Source = Source.Bing

  override def search(query: String, offset: Int, limit: Int): Future[SearchingResponseBody] =
    sttp.client3.basicRequest
      .get(uri"https://www.bing.com/api/search?q=$query&offset=$offset&limit=$limit")
      .send(server.stub)
      .map(_.body)
      .flatMap {
